...
y = 2;
        % img_8x8: img block to be DCT2D
        img_8x8_blk2 = img_grey_120x160(8*x-7:8*x,8*y-7:8*y);
        % DCT_8x8_cosLUT: 8x8 DCT kernel
        DCT_8x8_blk2 = myfunc_DCT2_8x8kernel_cosLUT(img_8x8_blk2, cosLUT);


% print input sequence
fileInputBlock = fopen('tb_DCT_8x8_MAC_input.txt','w');
% print first block input

for r = 1 : 64
    for m = 1 : 8
        for k = 1 : 8
            fprintf(fileInputBlock,"%s\n",dec2bin(img_8x8(m, k),8));
...
for r = 1 : 64
    for m = 1 : 8
        for k = 1 : 8
            fprintf(fileInputBlock,"%s\n",dec2bin(img_8x8_blk2(m, k),8));
        end
    end
end
fclose(fileInputBlock);
fprintf("tb_DCT_8x8_MAC_input.txt is written. \n");

% print DCT block output
fileOutputBlock = fopen('tb_DCT_8x8_MAC_output_ref.txt','w');
% print DCT block 1 output
for m = 1 : 8
    for k = 1 : 8
        fprintf(fileOutputBlock,"%s\n",bin(DCT_8x8(m, k)));
    end
end
% print DCT block 1 output
for m = 1 : 8
    for k = 1 : 8
        fprintf(fileOutputBlock,"%s\n",bin(DCT_8x8_blk2(m, k)));
    end
end
fclose(fileOutputBlock);
fprintf("tb_DCT_8x8_MAC_output_ref.txt is written. \n");
%%
DCT_8x8(1,1)
